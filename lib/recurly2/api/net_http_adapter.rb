    ...
    module Net
      module HTTPAdapter
        ...
        def request method, uri, options = {}
          ...
          request.basic_auth(*[Recurly3.api_key, nil].flatten[0, 2])
          if options[:body]
          ...
          if Recurly3.logger
            Recurly3.log :info, "===> %s %s" % [request.method, uri]
            headers = request.to_hash
            headers['authorization'] &&= ['Basic [FILTERED]']
            Recurly3.log :debug, headers.inspect
            if request.body && !request.body.empty?
              Recurly3.log :debug, XML.filter(request.body)
            end
            start_time = Time.now
          end
          ...
          if Recurly3.logger
            ...
            Recurly3.log :debug, response.to_hash.inspect
            Recurly3.log :debug, response.body if response.body
          end
